...
exports.handler = async (change, context) => {
    try {
        ...
        if(before && after) {
            ...
            await fb.db.collection(rest+'/'+restID+'/WarehouseStock/'+rawMaterial+'/Meals').get().then((docs) =>{
                ...
                if(bAmn < aAmn) {
                    ...
                    docs.forEach(doc => {
                        ...
                        if(!docVals.menu && parseFloat(docVals.redLine)  < aAmn) {
                            ...
                            batch.set(fb.db.doc(rest+'/'+restID+'/Meals/'+docID),{missing: {[rawMaterial]: fb.fieldValue.delete()}},{merge: true});
